...
## filter() za odabir redova po uvjetu {#sec-filter}
...
## select() za odabir i preimenovanje stupaca {#sec-select}
...
## mutate() za kreiranje i transformaciju varijabli {#sec-mutate}
...
### Čišćenje stupca spol pomoću str_to_lower() i case_when() {#sec-case-when}
...
## group_by() i summarise() za statistike po grupama {#sec-group-by}
...
## across() za istu operaciju na više stupaca {#sec-across}
...
Kad proslijedite imenovu listu funkcija, `across()` kreira zasebne stupce za svaku kombinaciju stupca i funkcije. Argument `.names = "{.col}_{.fn}"` kontrolira kako se novi stupci imenuju — `{.col}` je ime izvornog stupca, `{.fn}` je ime funkcije iz liste.
...
## pivot_longer() i pivot_wider() za preoblikovanje podataka {#sec-pivot}
...
### pivot_longer() za pretvaranje od širokog prema dugačkom formatu
...
### pivot_wider() za pretvaranje od dugačkog prema širokom formatu
...
## Spajanje tablica pomoću left_join() {#sec-join}
...
12. Svaka analiza ima jasne faze, a to su učitavanje, čišćenje, provjera, analiza i prezentacija. Dokumentirajte svaki korak i svaki izbor (osobito koliko redova gubite filtiranjem).
:::
