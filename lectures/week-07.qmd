...
Kad je p malo (0.05), distribucija je jako iskrivljena udesno i koncentrirana oko 0 i 1. Kad je p umjereno (0.20), distribucija se širi i centar se pomiče udesno. Kad je p = 0.50, distribucija je simetrična i izgleda gotovo poput zvona. Ova simetrija kod p = 0.5 nas vodi prema najvažnijoj distribuciji u cijeloj statistici, a to je normalna.
...
Normalna distribucija ima dva parametra. **μ (mi)** je srednja vrijednost (prosjek), koja određuje centar distribucije. **σ (sigma)** je standardna devijacija, koja određuje širinu distribucije.
...
4. Binomna distribucija modelira broj uspjeha u n nezavisnih pokušaja s vjerojatnošću p. R funkcije su `dbinom()`, `pbinom()`, `rbinom()`.
...
7. R funkcije za normalnu uključuju `dnorm()` za gustoću, `pnorm()` za kumulativnu vjerojatnost (P(X ≤ x)), `qnorm()` za kvantile (obrnuto od pnorm) i `rnorm()` za simulaciju.
