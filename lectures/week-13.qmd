---
title: "Tjedan 13: Linearna regresija"
...
Ovo je izuzetno važno. Zamislite da objave s više hashtagova također imaju duži tekst. U jednostavnoj regresiji, koeficijent za hashtagove upija oba efekta, što je problem. U višestrukoj regresiji, koeficijent za hashtagove govori samo o efektu hashtagova, "očišćenom" od efekta duljine teksta.
...
Tri mjere za usporedbu modela zaslužuju objašnjenje, a to su R-kvadrat, prilagođeni R-kvadrat i AIC.
...
Ovo se zove multikolinearnost, a pojavljuje se kad su prediktori međusobno jako korelirani. VIF, što je kratica za Variance Inflation Factor, mjeri koliko je varijanca koeficijenta narasla zbog korelacije s drugim prediktorima.
...
Linearna regresija pretpostavlja linearne odnose. Ali stvarni odnosi često nisu linearni - primjerice, tri hashtaga su vjerojatno bolja od nula, ali trideset hashtagova vjerovatno nije deset puta bolje od tri. Možda postoji optimalna točka, a sve iznad i ispod nje je lošije.
...
Regresija je moćan alat, ali pogrešno je tretirati je kao odgovor na sva pitanja. Četiri ograničenja zaslužuju ozbiljnu pozornost, uključujući činjenicu da korelacija nije kauzalnost, da je model dobar koliko i podaci, da je ekstrapolacija opasna i da pretpostavke moraju biti zadovoljene.
...
| Pojam | Objašnjenje |
|---|---|
| Linearna regresija | Modeliranje linearne veze između prediktora (X) i ishoda (Y) kroz jednadžbu Y = b₀ + b₁X + ε. |
...
| Rezidual (e) | Razlika između opaženog i predviđenog Y, što je označeno s e = Y − Ŷ. |
...
| Dummy varijabla | Binarna (0/1) varijabla za kodiranje kategorija, koju R automatski kreira u `lm()`. |
...
| `lm()` | R funkcija za linearnu regresiju, koristi se kao `lm(y ~ x1 + x2, data = ...)`. |
| `summary()` | Na `lm()` objektu daje koeficijente, SE, t, p, R² i F-test. |
| `predict()` | Generira predviđene vrijednosti pomoću `predict(model, newdata = ...)` za nova opažanja. |
| `residuals()` | Izvlači reziduale iz modela. |
| `broom::tidy()` | Pretvara model output u tibble s koeficijentima, SE, t, p i CI. |
| `AIC()` | R funkcija za izračun AIC-a, koristi se kao `AIC(model1, model2)` za usporedbu. |
